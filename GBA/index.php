...
font-weight: bold;
   font-size: 9px;
            ...
            body {
background: url(ico/gba.png) no-repeat  -50px 250px;
                font-family: monospace;
                font-weight: bold;
                font-size: 12px;
                margin: 0;
...
<center>
<div id = stuff>
<h1>محطة ألعاب جيم بوي ادڤانس</h1>
</div>
<script>
</script>
	<div id = box>
...
{ if ((substr($file, -3)=="bin")  ||(substr($file, -3)=="z64") || (substr($file, -3)=="gb") || (substr($file, -3)=="zip") ||(substr($file, -3)=="nes") || (substr($file, -3)=="n64") || (substr($file, -3)=="gbc") || (substr($file, -3)=="smd") || (substr($file, -3)=="gba") || (substr($file, -3)=="bi1") ||(substr($file,
-3)=="smc"))
{ $imgArray[ ] = trim($file);
}
} $dirPath->close();
...
$c = count($imgArray); for($i=0; $i<$c; $i++) { echo "<option value=\"" . $imgArray[$i] . "\">" . $imgArray[$i] . "\n";
     ...
     <button id = input value="Mortal Kombat Deadly Alliance.zip" onclick="go();">ألعب</button>       
