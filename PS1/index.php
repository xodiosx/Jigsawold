...
font-weight: bold;
   font-size: 9px;
            ...
            body {
background: url(ico/ps1.png) no-repeat  -50px 250px;
                font-family: monospace;
                font-weight: bold;
                font-size: 12px;
                margin: 0;
...
{ if ((substr($file, -3)=="bin")  ||(substr($file, -3)=="cue") || (substr($file, -2)=="7z") || (substr($file, -3)=="zip") ||(substr($file, -3)=="chd") || (substr($file, -3)=="n64") || (substr($file, -3)=="ccd") || (substr($file, -3)=="toc") || (substr($file, -3)=="gba") || (substr($file, -3)=="ps1") ||(substr($file,
-3)=="m3u"))
{ $imgArray[ ] = trim($file);
}
} $dirPath->close();
...
$c = count($imgArray); for($i=0; $i<$c; $i++) { echo "<option value=\"" . $imgArray[$i] . "\">" . $imgArray[$i] . "\n";
     ...
     <button id = input value="crash.ps1" onclick="go();">ألعب</button>       
...
var url = input.value;
       ...
       var extension = url.split(".").pop().toLowerCase();
					...
					if (extension=="nes")
							...
							else if (extension=="bin" || extension=="cue")
								{
							 core = "psx"; 
								}
	...
	else	if (extension=="ccd" || extension=="chd" || extension=="7z")
								{
							 core = "psx";
								}
...
window.EJS_language = 'ar-AR';
                ...
                window.EJS_biosUrl = "bios/psxbios.7z"             
window.EJS_CacheLimit = 9973741824
...
var text = e.options[e.selectedIndex].text;
	...
	<h2>brought to you by M.Aleiwi</h2>
</div>
</center>
</html>
